...
function X = PAC(Xc, Xs, Pt, rho)
% Solution to Per-Antenna Power-Constrained (PAC) Waveform Synthesis
%       min   rho * || X - Xc ||^2_F + (1 - rho) * || X - Xs ||^2_F
%       s.t.  diag(X*X'/L) = I * Pt/N
% where Xc is the perfect waveform for communication and Xs is the perfect waveform for sensing
    ...
    % The function below solves (26) of [Liu et al; 2018; TSP; 10.1109/TSP.2018.2847648]
    X = JointWaveformDesign_PAC(eye(N), Xc, Xs, Pt, rho);  
end

function X = JointWaveformDesign_PAC(H, S, X0, Pt, rho)
%--------------------------------------------------------------------------
% Shixiong Wang's Note:
% This function is adapted from "helperRadComWaveform" at...
%      https://uk.mathworks.com/help/phased/ug/waveform-design-for-a-dual-function-mimo-radcom-system.html
% NB: I did a small modification because there was a bug inside.
%--------------------------------------------------------------------------
%
%
...
%{{{{-------- Begin of Shixiong Wang's Modification
    if abs(rho) < 1e-6
        A = eye(N);
        B = X0;
    elseif abs(rho - 1) < 1e-6
        A = H;
        B = S;
    else
...
%}}}}-------- End of Shixiong Wang's Modification
